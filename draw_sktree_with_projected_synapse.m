...
function draw_sktree_with_projected_synapse( tree_with_synapse , seg_size_in , output_file_name , tree_ids )
...
for i = 1 : numel( tree_with_synapse )
    ...
    % draw skeleton and annotate synapse
    for j = 1 : numel( this_tree.nodes )
        ...
        for k = 1 : numel( this_node.children )
            ...
            branch_seg_id = this_tree_id; 
            
            p2 = uint32([next_node.voxel_x next_node.voxel_y next_node.voxel_z]);
            [line_segment, min_coord, max_coord] = get_line_segment(p1, p2, branch_seg_id, 0);
            ...
            if next_node.spine
                ...
                annotation_name = next_node_id; 
                annotation_color = color_code( next_node.spine + 1 , : );
                annotation_data = [ annotation_data; uint32( [annotation_id next_node.spine annotation_name p2 annotation_color] ) ];  %#ok<AGROW>
            end
        end
